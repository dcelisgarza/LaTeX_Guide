...
\begin{verbatim}
	...
	\date{<date>}
\end{verbatim}
The date can be dynamically updated to the OS' by using \cs{today}
as the \cs{date} command's argument.  The affiliation is added as
a footnote by the \cs{thanks\marg{}} command.
...
\cs{maketitle} to be inside the document environment.
...
\ref{t:page}.  The \cs{pagenumbering\marg{style name}} changes the
...
\ref{t:pagestyle}.  They are declared within the \cs{pagestyle\marg{}}
(all pages), or \cs{thispagestyle\marg{}} (this page only) commands.
\begin{table}[!htbp]
    \centering
...
\begin{itemize}
    \item \cs{setlength\marg{command}\marg{length}}: set a length command to a new value \meta{length}.
    \item \cs{addtolength\marg{command}\marg{length}}: add a value, \meta{length}, to a length parameter.
    \item \cs{linespread\marg{len}}: set inter-line spacing to the value \meta{length}.
\end{itemize}
...
placing the factor before a specific \cs{lengthcommand},
e.g. \verb|-0.5\textwidth| divides the text width by $-2$.

There are \cs{lengthcommand}s for all manner of things, from margin
...
The most used by far are \cs{textwidth}---text width, which is
often different from the page width due to text padding and
multicolumns---and \cs{width}---page width with margins---because
...
is \emph{always} \cs{begin{document}} and the \texttt{.tex} file's
very last line is \emph{always} \cs{end{document}}.
...
\begin{itemize}
\item \cs{part\marg{part name}}: Parts. Primarily used in book writing.
\item \cs{chapter\marg{chapter title}}: Chapters.
\item \cs{section\marg{section title}}: Sections.
\item \cs{subsection\marg{subsection title}}: Subsections.
\item \cs{subsubsection\marg{subsubsection title}}: Subsubsections.
\end{itemize}
%
