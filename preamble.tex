...
\usepackage{fontspec}		  % Font package for XeLaTeX. It has native accent 
				  % support. No more editing .bib files to replace 
				  % missing unicode accents for their backslash TeX version.
...
\graphicspath{{./images/}}        % Relative image path. Keep your stuff organised. (N) dots before 
				  % the first forward slash means you go up (N-1) directories before 
				  % looking for the path after the slash.
