...
options,
\cs{begin{environment}[options]}\footnote{\url{https://en.wikibooks.org/wiki/LaTeX/List_Structures}}.
  The \verb|enumitem| package expands this capability.
%
\subsection{Itemize}
%
The \verb|itemize| environment lists items as bullet-point lists.
Bullet points can be redefined with \cs{renewcommand{}{}} or with
functionality offered by the \pkg{enumitem} package.  The environment
is used as follows:
\begin{verbatim}
	\begin{itemize}
...
numbering can be redefined with \cs{renewcommand{}{}} or with
functionality offered by the \pkg{enumitem} package.  It's declared
as follows:
\begin{verbatim}
	\begin{enumerate}
    	\item Nesting level 1.
...
%
There are a couple of ways to do this, with either the \pkg{paralist}
or \pkg{enumitem} package.  For \pkg{paralist}, the counter tokens
are A, a, I, i and 1; and respectively represent the \cs{Alph},
\cs{alph}, \cs{Roman}, \cs{roman}, and \cs{arabic} counter
styles.  Conversely, \pkg{enumitem} uses them explicitly.  Both
environments can take optional font shape switches and commands, as
well as other symbols for item label customisation.  In
\pkg{paraenum} this is done like so:
\begin{verbatim}
	...
	\end{inparaenum}
\end{verbatim}
in \pkg{enumitem} the way to do this is:
\begin{verbatim}
...
\pkg{verbatim} package is essential.
%
...
\cs{verb} is the delimiter---which can be any character except for *.  The \env{alltt}
