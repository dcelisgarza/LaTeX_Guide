...
non-mathematicians use.  The packages \pkg{mathtools}, \pkg{bm},
\pkg{amssymb}, and \pkg{pxfonts} respectively provide mathematical
...
\cs{text\marg{symbol name}} or \cs{text\marg{command name}}.
%
...
printed within a mathematical environments use the \cs{text{}}
...
the \cs{boldsymbol{}} command.  When a bold symbol does not exist
there's the \cs{pmb{}} (poor man's bold) which prints slightly
...
\cs{arc\marg{function}}.  In fact, most functions are defined exactly
...
solved by the nest-able \cs{mathlarger{}} command, provided by the
\pkg{relsize} package.  The number of integrals is defined by the
...
ones are provided by the \AmSTeX~packages.  The \pkg{mathdots}
...
\cs{dots} command.  There also exist dotless versions of $\imath$
and $\jmath$, provided by \cs{imath} and \cs{jmath}
respectively---useful for properly adding hats to orthogonal unit
vectors.

Some accents also have wide versions.  Some bar accents (including
arrows) are denoted as \cs{over\marg{bar accent}}, while other wide
accents are denoted as \cs{wide\marg{accent}}.  Table \ref{t:ma}
...
Furthermore, \cs{overbrace{}} and \cs{underbrace{}} may have
sub/superscripts automatically placed directly below and above them
without resorting to %@todo \cs{limits\sub{}\sup{}}.  For example:
\begin{verbatim}
...
\cs{mathclap{}} command will eliminate the extra spacing.
%
\subsection{Limits and Substack Commands}
%
Some symbols may have arguments and/or sub/supersripts in different
places.  Placing the \cs{limits} command immediately after a
...
requirement.  The \cs{substack} command allows subscripts to be
...
delimiters, these are obtained by adding \cs{left}, \cs{right}
and \cs{middle} (optional) before the actual delimiter, they also
...
print them, \cs{left\{} \cs{right\}}.  Sometimes the delimiter is
...
\pkg{bm} and \pkg{mathtools} packages cover most other cases.  Table
...
equations can be labeled by placing \cs{label{<label>}} before a
line break, and referenced with \cs{eqref{}}.  Individual equation
numbers can be removed with the \cs{nonumber} command before a line
...
can also be customised via \cs{renewcommand{}{}}.  Using the
...
\pkg{mathtools} package.  The default types are shown in table
...
functions without the pain of recurring to \cs{aligned} and
