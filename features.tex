...
environments.  The most obvious example of this is the \cls{beamer}
class, which automatically loads the \pkg{hyperref} package and
provides custom commands and environments such as \cs{frametitle}
and \env{frame}, respectively.

The following are common options of the \cs{documentclass} command,
...
\item Document language: \verb|german|, \verb|greek|, \verb|spanish|,
  \textbf{\texttt{english}}.  Can be overridden with the \pkg{babel}
  and \pkg{polyglossia} packages.
...
\begin{itemize}
\item \pkg{babel}: sets punctuation (hyphenation), part, chapter,
  section, subsection, subsubsection, figure, subfigure, table,
  etc. to the language of your choice.  \pkg{polyglossia} is the
  \XeLaTeX~alternative, but \pkg{babel} still has more regional
  options such as \verb|[spanish,mexico]|.
\item \pkg{fontspec}: loads utf-8 encoded fonts, natively supports
  accents---even in \texttt{.bib} files---and works on \XeLaTeX~and
  \LuaTeX.  It replaces PDF\LaTeX's \pkg{inputenc} and \pkg{latin}
...
\begin{verbatim}
	...
	\end{environment}
\end{verbatim}
while commands are simply written as \cs{command}.  Options and
...
text, e.g. \cs{command~text}.  Punctuation can be placed
immediately following a command without problem.

Some commands and environments have \emph{starred} versions, where an
asterisk, *, is placed immediately after their name \cs{command*},
\cs{begin{environment*}} and \cs{end{environment*}}.  Starred
...
\pkg{lmodern} package at the very end of the package declaration
...
There are predefined font sizes whose absolute size varies depending
...
%
\cs{input{}} and \cs{include{}} are two \emph{very} special and
...
\cs{input{filename}} is very low level, and it simply inserts
...
\cs{include{filename}} is a bit more complicated and \emph{much}
more useful when it comes to sectioning.  This command forces a
\cs{clearpage} before and after inserting \texttt{filename.tex}'s
...
Calling a counter is done with the command \cs{the\meta{counter name}},
which prints the counter as it is defined, and can be used to create
custom formats with \cs{renewcommand{}{}} (more on renewing
commands later).  The command \cs{value\marg{counter name}} will
...
\cs{\meta{counter style}\marg{counter name}}, and has to be done within
\cs{renewcommand{}{}} so the changes are global.
\begin{table}[!htbp]
  \centering
...
As an example, say we want our pages to be numbered as follows
...
done with \cs{stepcounter\marg{counter name}}.  Adding or subtracting
an arbitrary number to a counter is done with
\cs{addtocounter\marg{counter name}{<number>}}.  Setting a counter to
a given number is done with
\cs{setcounter\marg{counter name}{<number>}}.  Setting a counter in
relation to some other counter is done with
\begin{verbatim}
	\setcounter{<counter_name>}{\number\value{<other_counter_name>}}
\end{verbatim}
They can also be created with the \cs{newcounter\marg{counter name}}
command.  And counters which are reset to zero every time another
counter is increased are created with
\cs{newcounter\marg{counter name}[\marg{trigger counter}]}.  New counters
must be created in the preamble.
%
\section{Table of Contents, Appendix, Index \& Glossary}\label{s:contents}
%
If they are used within the \cls{beamer} document class, remove all
paging commands.
...
\end{verbatim}
The \cs{tableofcontents} command prints the table of contents,
\cs{listoftables} prints a list of tables, \cs{listoffigures}
prints one for figures, and
\cs{addcontentsline{toc}{\marg{section type}}{\marg{name}}} adds an entry to
...
\cs{cleardoublepage} and \cs{phantomsection} commands ensure the
...
Anything can be added to the table of contents with the
\cs{addcontentsline{}{}{}} command, including appendices, indices,
and unnumbered (starred) sectioning commands.  Glossaries can be added
with the \pkg{glossaries} package.
%
\subsection{Appendix}
%
Adding an appendix is as easy as writing \cs{appendix} where the
appendix is to be printed.  It only needs to be added once, all
subsequent sectioning commands will be formatted accordingly.
%
\subsection{Index}
%
Creating an index requires the \pkg{makeidx} package, adding the
\cs{makeindex} command in the preamble, and placing
\cs{printindex} where the index is to be printed.  Index entries
are specified with the \cs{index{}} command.  Table \ref{t:index}
...
added as \cs{<accent>} followed by the word to be indexed.
...
\subsection{Glossary}
%
The glossary requires the \pkg{glossaries} package, and adding the
\cs{makeglossaries} in the preamble.  Added functionality can be
added by specifying the package options: \pkg{xindy}, which offers
better indexing than \pkg{makeindex} (default); and \pkg{toc}, which
...
within \cs{ensuremath{}}.  The description can have in-line maths.
The \cs{longnewglossaryentry{}{}{\marg{long description}}} command
...
is done with the
\cs{newacronym{\marg{label}}}{\marg{abbreviation}}{\marg{full word}} command.  If
acronyms require a different list, add the \opt{acronym} option to
the \pkg{glossaries} package.

Referencing glossary entries is done with \cs{gls\marg{label}}, by
adding symbol, desc and pl between \cs{gls} and the argument, it
...
\cs{glslink{\marg{label}}{\marg{alternate text}}}.

There is much more information in the documentation for the
\pkg{glossaries} package and the \LaTeX~wikibook's glossary
...
All macros must be defined in the preamble.  Commands can be defined
...
\end{verbatim}
respectively produce the commands \cs{helwo12{}{}} and
\cs{helwo21{}{}}.  If one were to write
\cs{helwo12{Hello!}{World!}}, the output would be ``Hello!
World!'', but if one were to write \cs{helwo21{Hello!}{World!}} the
...
label---\cs{label{}}---and reference---\cs{ref{}} for
non-equations, and \cs{eqref{}} for
...
simultaneously loaded.  Fortunately \pkg{mathtools} is vastly
superior to \pkg{amsmath} in every respect, and does \emph{not}
present this bug; so use it in place of \pkg{amsmath} and enjoy the
wonders of \pkg{cleveref}.
%
...
extensions.  The \pkg{babel} and
\pkg{polyglossia}\footnote{Preferred for \XeLaTeX.} packages provide
...
portugues, etc.  Unfortunately \pkg{polyglossia} still lacks some
regional adjustments, including latin-american and mexican spanish,
which means many users should stick to \pkg{babel} for the time
...
be found in \pkg{babel}'s user manual.
%
...
\end{itemize}
They can be referenced using the \cs{cite[]{}}, where the optional
...
to be placed here.  Though I'd like to mention \cs{nocite{}},
...
the document's content and before \cs{end{document}}:
\begin{verbatim}
