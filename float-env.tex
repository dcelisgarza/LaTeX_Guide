...
Along with these, there's the command, \cs{FloatBarrier}, which
forces all previous floats to be rendered before it.  And the
\pkg{placeins} package, often used with the \verb|section| option,
...
\cs{caption{}}.  As a rule, table captions in tables are found at
the top, while figure captions at the bottom.  There is also a starred
version, \cs{caption*{}}, that eliminates the float number, useful
when tables or figures need notes.  In order to reference a float, it
must be assigned a label, \cs{label{}}, after the caption.  If a
float has no caption but needs a label, an empty caption can be added
before labeling, \cs{caption{}\label{label}}\footnote{Do \emph{not}
...
so the \cs{centering} alignment switch is usually the first line in
...
reserving the whole page width (the value of \cs{width}) for the
...
\cs{includegraphics[]{}} can be used on its own to include in-line
...
\end{enumerate}
The package \pkg{booktabs} provides improved versions of the
horizontal line commands, \cs{hline} and
\cs{cline{}}.\footnote{If there's a need for non-tabular horizontal
  lines---such as redefining footnote rules---you should still use
  \texttt{\textbackslash hline}.}  They are: \cs{toprule},
\cs{midrule}, \cs{cmidrule{}}, and \cs{bottomrule}.  They all
...
\cs{cmidrule{i-j}} which spans columns $i$ to $j$.  The
\pkg{array} package extends table functionality by providing extra
...
The documentation for \pkg{booktabs} contains further information on
...
(sometimes both).  When this is unavoidable, the \pkg{multirow}
...
\end{verbatim}
An asterisk ``*'' as width in \cs{multirow} stands for the
contents' natural width.  \cs{multicolumn} gobbles all relevant
columns, so there's no need to add the gobbled ampersands.  If
\cs{multirow} is used, all subsequent rows in the same column(s)
...
page.  For cases such as these, the \pkg{longtable} package provides
...
\begin{verbatim}
	...
	\end{longtable}
\end{verbatim}
\pkg{longtable} also works with the functionality offered by the
\pkg{multirow} package.  However, multiple compilations ($4+$) may be
